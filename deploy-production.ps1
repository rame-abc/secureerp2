...
try {
    ...
    $dockerRunResult = docker run -d `
        --name secureerp2-production `
        --restart unless-stopped `
        -p 5000:5000 `
        -p 5001:5001 `
        -e ASPNETCORE_ENVIRONMENT=Production `
        -e ConnectionStrings__DefaultConnection="Server=production-db-host;Database=SecureERP2_Production;User Id=secureerp_user;Password=PROD_PASSWORD_PLACEHOLDER;TrustServerCertificate=true;Encrypt=true;" `
        -e Authentication__Jwt__SecretKey="PROD_JWT_SECRET_KEY_PLACEHOLDER_MINIMUM_32_CHARACTERS" `
        -v "/c/data/secureerp2/logs:/app/logs" `
        -v "/c/data/secureerp2/backups:/app/backups" `
        --memory 2g `
        --cpus 1.0 `
        secureerp2:production
